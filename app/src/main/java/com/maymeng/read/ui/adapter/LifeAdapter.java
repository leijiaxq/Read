...
import com.maymeng.read.ui.activity.BookDetailActivity;
...
public class LifeAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private List<BookBean.BooksBean> mDatas;
    public boolean isAllLoad = false;


    public LifeAdapter(Context context, List<BookBean.BooksBean> datas) {
        mContext = context;
        mDatas = datas;
    }
    ...
    private void setDataType1(ViewHolderType1 holder, final int position) {
        final BookBean.BooksBean bean = mDatas.get(position);
        ...
        holder.mItemScoreTv.setText(TextUtils.isEmpty(bean.rating.average)?"":bean.rating.average);

        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(mContext, BookDetailActivity.class);
                intent.putExtra("book_id", bean.id);
                intent.putExtra("book_url", bean.images.medium);
